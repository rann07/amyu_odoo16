...
class DtrByEmployee(models.Model):
    _name = "dtr.by.employment.status"
    _description = "DTR by Employment Status Wizard"
    ...
    def button_export_xls(self):
        ...
        url = inet_host + "/?report=" + inet_folder + \
              "/dtr_by_employee.rpt&init=xlsx&prompt0=" + str(company_id) + \
              "&prompt1=" + from_date + "&prompt2=" + to_date
        return {'name': 'DTR by Employee',
    ...
    def button_preview_report(self):
        ...
        to_date = self.to_date.strftime("Date(%Y,%m,%d)")
        url = inet_host + "/?report=" + inet_folder + \
              "/dtr_by_employment_status.rpt&init=pdf&prompt0=" + str(company_id) + \
              "&prompt1=" + from_date + "&prompt2=" + to_date
        return {'name': 'DTR by Employment Status',
                'res_model': 'ir.actions.act_url',
                'type': 'ir.actions.act_url',
                'target': 'new',
                'url': url
